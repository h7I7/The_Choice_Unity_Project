...
public class InteractionController : MonoBehaviour
{
    ...
    [SerializeField]
    private float m_trackingSpeed = 20;
            ...
            case InteractionState.None:
                {
                    ...
                    if (hitInf.collider != null)
                    {
                        ...
                        // Activating the butler chase at end game
                        if (hitInf.collider.tag == "ButlerChase")
                        {
                            ActivateButlerChase();
                        }
                    }
            ...
            case InteractionState.Talking:
                {
                    ...
                    if (Input.GetMouseButtonDown(0) && m_dialogueEnum == dialogueEnumerator.Talking)
                    {
                        m_skipSentance = true;
                    }
            ...
            case InteractionState.Lifting:
                {
                    ...
                    if (!Input.GetMouseButton(0))
                    {
                        ...
                        EndPickup();
                        hitInRange.collider.gameObject.GetComponent<Rigidbody>().AddForce((pos - hitInRange.transform.position) * (m_trackingSpeed * 0.25f), ForceMode.VelocityChange);
                        break;
                    }
                    ...
                    if (Input.GetMouseButtonDown(1))
                    {
                        ...
                        GetComponent<PlayerController>().m_stopMovement = false;
                        // Throwing object
                        hitInRange.collider.GetComponent<Rigidbody>().velocity = m_playerCamera.forward.normalized * (m_throwForce * 0.5f);
                    ...
                    hitInRange.transform.Translate((pos - hitInRange.transform.position) * Time.deltaTime * m_trackingSpeed, Space.World);
    ...
    private UnityEvent m_AfterDialogueActions;
    ...
    public void StartPickup()
    {
        ...
        m_heldDist = Vector3.Distance(m_playerCamera.position, hitInRange.transform.position);
    ...
    #region Butler Chase
    private void ActivateButlerChase()
    {
        hitInf.collider.gameObject.GetComponent<ButlerMover>().ActivateChase();
    }
    #endregion
    ...
    {
        FadeManager.Instance.FadeOutCanvas(true);
    }
    #endregion
}